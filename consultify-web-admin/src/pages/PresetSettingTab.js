...
import {  salonDetailsAction, salonPresetCreatedByAdminUser, updateConsultationPresetStatus, updateConsultationStatus, updatePreAndPostCareStatus, updatePrePostCareStatus } from '../Redux/Action/SalonAction';
import { useDispatch, useSelector } from 'react-redux';
import moment from 'moment';
import { Form } from 'react-bootstrap'

export default function PresetSettingTab() {
    const { id } = useParams();
    ...
    var { salondetails, salonpresetlist } = useSelector((state) => ({
        salondetails: state.Salon.salondetails,
        salonpresetlist: state.Salon.salonpresetlist
    }));

    useEffect(() => {
        dispatch(salonDetailsAction(idvalue))
        dispatch(salonPresetCreatedByAdminUser(idvalue))
    }, [idvalue])

      useEffect(() => {
        if (salonpresetlist?.length > 0) {
          setSwitchStates(
            salonpresetlist.reduce((acc, item) => {
              acc[item._id] = item?.hasOwnProperty("see_postcare")
                ? item.see_postcare
                : 1;
              return acc;
            }, {})
        ...
        if (salonpresetlist?.length > 0) {
            setSwitchStatesPreset(
              salonpresetlist.reduce((acc, item) => {
                acc[item._id] = item?.hasOwnProperty("see_precare_presets")
                  ? item.see_precare_presets
                  : 1;
                return acc;
              }, {})
            );
    ...
    const handleToggle = (consultationId) => {
        ...
        dispatch(
            updatePreAndPostCareStatus({
                // id: idvalue,
                salon_id: idvalue,
                see_postcare: updatedState,
                preset_id: consultationId
            })
        );
    };
    ...
    const handleTogglePreset = (consultationId) => {
        ...
        dispatch(
            updatePrePostCareStatus({
                see_precare_presets: updatedState,
                preset_id: consultationId,
                salon_id: idvalue,
            })
        );
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center flex-wrap mb-3 position-relative">
                                                <h4 class="title_text">Presets</h4>
                                            </div>
                                                ...
                                                <table id="" class="table dt-responsive dealers_table nowrap w-100">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th className="hide-on-small">Pre & Post Treatment Care Tab</th>
                                                            <th className="hide-on-small">Pre & Post Care Presets Tab</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="td_color">
                                                        {
                                                            salonpresetlist?.map((item, i) => {
                                                                return (
                                                                    <>
                                                                        <tr>
                                                                            <td>{item?.treatmentname}</td>
                                                                            <td className="hide-on-small">--</td>
                                                                            <td>{item.hasOwnProperty("admin_id") ? "Admin" : "Salon"}</td>
