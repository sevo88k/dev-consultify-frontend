...
export default function Forum() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (idvalue) {
        values.id = idvalue;
        dispatch(updateForum(values)).then((data) => {
        ...
        dispatch(createForum(values)).then((data) => {
          if (data?.payload?.success) {
            dispatch(
              getForum({
                page: 1,
  ...
  const sideeffectlists = useSelector((state) => state.Forum.forums);
      ...
      <div id="layout-wrapper">
                  ...
                  <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <div>
                      ...
                      <div className="d-flex justify-content-center align-items-center">
                        <h4 class="mb-sm-0 font-size-28">Forums</h4>
                        <p className="total"></p>
                      </div>
                    </div>
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>
            <form className="sideffect_popup" onSubmit={formik.handleSubmit}>
              <h2>{idvalue ? "Edit" : "Add"} Forum</h2>

              <div className="form-group">
                <label for="exampleInputEmail1">
                  {idvalue ? "Edit" : "Add"} Forum
                </label>
                <textarea
                  class="form-control"
        ...
        >
          ...
          <Modal.Body className="my_new_consult">
            <h2 className="text-center">Are you Sure to delete this Forum</h2>
            <div className="cent_btns">
              <button
                onClick={() => {
                  dispatch(deleteForum(idvalue)).then(function () {
                    dispatch(
                      getForum({
                        page: 1,
                        limit: 1000000000000,
